...
$sql = $QRY["ocorrencias_full_ini_count"] . " WHERE ua.AREA = " . $_SESSION['s_area'] . " and s.stat_painel in(3) AND o.data_abertura > date_sub(CURRENT_DATE, INTERVAL {$months} month) ";
$sqlResult = $conn->query($sql);
$totalData = $sqlResult->fetch()['total'];
$totalFiltered = $totalData;  // when there is no search parameter then total number rows = total number filtered rows.


$sql = $QRY["ocorrencias_full_ini"] . " WHERE ua.AREA = " . $_SESSION['s_area'] . " and s.stat_painel in(3) AND o.data_abertura > date_sub(CURRENT_DATE, INTERVAL {$months} month) ";
